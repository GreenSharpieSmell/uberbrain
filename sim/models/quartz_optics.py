...
Functions:
  ...
  rayleigh_range()       — focal depth limit (validates E. Colty's calculation)
...
def rayleigh_range(
    ...
    Validates E. Colty's calculation: at NA=0.5, λ=1030nm → z_R ≈ 1.9µm
...
def layer_count_estimate(
    ...
    Validates: E. Colty's result → 10mm disc, 1030nm, NA=0.5 → ~2000 layers
    """
    if adaptive_optics:
        safety_factor = 1.3
    ...
    return {
        ...
        "ecolty_validated":       abs(int(n_layers_practical) - 2000) < 200,
    }
...
def validate_against_ecolty() -> dict:
    """
    Validate this model against E. Colty's Rayleigh range calculation.

    E. Colty's result (2026-03-23):
    ...
    # E. Colty said z_R ≈ 1.9µm and ~2000 layers
...
if __name__ == "__main__":
    ...
    print("\n── Rayleigh range (validates E. Colty) ─────────────────────")
    val = validate_against_ecolty()
    print(f"  z_R computed: {val['computed_zr_um']:.4f}µm  "
          f"(E. Colty: {val['ecolty_zr_um']}µm)  "
          f"{'✓' if val['zr_match'] else '✗'}")
    print(f"  Layers:       {val['computed_layers']}  "
          f"(E. Colty: {val['ecolty_layers']})  "
