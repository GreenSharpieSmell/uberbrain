# Contributors

## Founding Session — March 23, 2026

### Rocks D. Bear
Human architect. Originated the core intuitions: light-based storage, wavelength encoding, the hand warmer / crystallization analogy for neuromorphic pathways, the oomphlap unit, the multi-color multitasking insight, the question of athermal switching. The connective thread across every idea in this repository.

### Claude (Anthropic)
AI reasoning system. Grounded intuitions in physics, identified named equivalents in existing research (PIPC, reservoir computing, LTP, stepped leader pathfinding, Project Silica), formalized the instruction set, wrote and maintained the technical documentation, calculated the Rayleigh range layer count, designed the self-correction architecture.

### Gemini (Google)
AI reasoning system. Independently converged on the athermal PIPC mechanism, named the Solid-State Optical Brain, coined the Consolicant, designed the transparency MVP experiment, identified the femtosecond pulse timing as the key variable, framed the AI safety implications of physics-based verification.

### Engineer Collaborator
Engineer collaborator. Contributed technical validation during the founding session: identified the MLC partial crystallization mechanism, independently derived the two-state GST limitation and its architectural solution, and checked the Rayleigh range parameters.

### Codex (OpenAI)
AI reasoning system. Independent technical reviewer. Identified the engineering feasibility gaps (femtosecond source miniaturization, abstract simulation vs hardware-feasibility modeling, absence of falsifiable KPIs). Authored `VALIDATION_SPEC.md` — the formal validation program that defines what evidence is required before strong claims can be made. Wrote the initial pytest suite in `tests/test_simulations.py`.

---

## How To Contribute

This repository is CC0 — public domain. Fork it, build on it, push it forward.

If you make progress on any of the open research threads in REFERENCE.md, open a pull request or an issue. The questions log in QUESTIONS_LOG.md is the right place for technical discussion.

The goal is not credit. The goal is that someone builds it.
